...
```{r context="server"}
unique_ind <- reactive({
    nes_st0418_p[nes_st0418_p$Industry.Title %in% input$big_ind & nes_st0418_p$ST_n %in% input$ch_state & nes_st0418_p$Year %in% input$yr,]
})
...
output$seg.plot <- renderPlotly({
  ...
  if(input$count_type == 'Total_RCPTOT'){
    ggplotly(ggplot(data = unique_ind(), mapping = aes(x = reorder(Industry.Title, -Year), xend = reorder(Industry.Title, -Year), y = Year, yend = 0, color = Industry.Title)) + 
    geom_point() +
      geom_segment()+
    coord_flip() + 
